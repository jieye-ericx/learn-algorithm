...
func Constructor(timeToLive int) AuthenticationManager {
	AuthenticationManager := AuthenticationManager{
		...
		currentTimes: make(map[string]int),
	}
...
func (this *AuthenticationManager) Generate(tokenId string, currentTime int) {
	this.currentTimes[tokenId] = currentTime
}

func (this *AuthenticationManager) Renew(tokenId string, currentTime int) {
	if _, ok := this.currentTimes[tokenId]; ok && this.timeToLive+this.currentTimes[tokenId] > currentTime {
		this.currentTimes[tokenId] = currentTime
	}
}
...
func (this *AuthenticationManager) CountUnexpiredTokens(currentTime int) int {
	count := 0
	for k, v := range this.currentTimes {
		if currentTime-v < this.timeToLive {
			count++
		} else {
			delete(this.currentTimes, k)
		}
	}
	return count
}
